...
import { JobCardDetailComponent } from 'src/app/shared/dialogs/project/job-card-detail/job-card-detail.component'
// import { JobCardDataSource, JobCardItem } from './job-card-datasource';
...
export class JobCardComponent implements AfterViewInit {
	...
	loadData(){
		this.spinner.show()
		this.auth.getAPI('job-card/?'+this.nextPage+'&project__slug='+this.projectSlug).subscribe(responce=>{
	...
	viewJobCard(jcdata:any){
		const dialogRef = this.dialog.open(JobCardDetailComponent,{
			width:'100%',
			data:{
				jcdata:jcdata
			}
	...
	projectBom(data:any){
		this.router.navigate(['/project/job-card/list/'+data])	
	}
}
